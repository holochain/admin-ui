...
async function testSource(secretKey, source) {
    ...
    const contract = await WalletContract_1.WalletContract.create(client, source);
    console.log('testing contract: ' + source.type + ' at ' + contract.address.toFriendly());
    let treasureSeqno = await treasure.wallet.getSeqNo();
    await treasure.wallet.transfer({ to: contract.address, seqno: treasureSeqno, value: (0, convert_1.toNano)(0.1), secretKey: treasure.secretKey, bounce: false });
    ...
    await (0, awaitCondition_1.awaitCondition)(30000, async () => (await contract.getSeqNo()) > 0);
}
