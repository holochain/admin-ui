...
exports.serializeToBoc = exports.deserializeBoc = exports.hashCell = void 0;
...
async function hashCell(cell) {
    return await (0, ton_crypto_1.sha256)(await getRepr(cell));
}
...
async function serializeForBoc(cell, cellsIndex, refSize) {
    const reprArray = [];
    reprArray.push(getDataWithDescriptors(cell));
    for (let k in cell.refs) {
        const i = cell.refs[k];
...
async function serializeToBoc(cell, has_idx = true, hash_crc32 = true, has_cache_bits = false, flags = 0) {
    ...
    const serialization = __1.BitString.alloc((1023 + 32 * 4 + 32 * 3) * topologicalOrder.length);
