...
function parseStyle(document, style, code, locationCalculator, parserOptions, cssOptions) {
    let textStart = 0;
    for (const { range, expr, exprOffset, quote, comments } of iterateVBind(code, cssOptions)) {
        ...
        const beforeTokens = [
            createSimpleToken("HTMLText", container.range[0], container.range[0] + 6, "v-bind", locationCalculator),
            createSimpleToken("Punctuator", container.range[0] + 6, container.range[0] + 7, "(", locationCalculator),
        ];
        const afterTokens = [
...
function* iterateVBind(code, cssOptions) {
    ...
    while ((match = re.exec(code))) {
        ...
        if (startOrVBind === '"' || startOrVBind === "'") {
            ...
            re.lastIndex = skipComment(code, startOrVBind === "/*" ? "block" : "line", re.lastIndex);
        }
        else {
...
function parseVBindArg(code, nextIndex, cssOptions) {
    ...
    while ((match = re.exec(code))) {
        ...
        if (startOrVBind === '"' || startOrVBind === "'") {
            const start = match.index;
            const end = (re.lastIndex = skipString(code, startOrVBind, re.lastIndex));
            stringRanges.push([start, end]);
        }
        else if (startOrVBind === "/*" || startOrVBind === "//") {
            const block = startOrVBind === "/*";
            const start = match.index;
            const end = (re.lastIndex = skipComment(code, block ? "block" : "line", re.lastIndex));
            comments.push({
...
function skipComment(code, kind, nextIndex) {
